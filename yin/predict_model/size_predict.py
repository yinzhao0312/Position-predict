...
outFile1 = codecs.open(path + 'size_rf.txt', 'w', 'utf-8')
outFile2 = codecs.open(path + 'size_gbdt.txt', 'w', 'utf-8')
...
def train():
    ...
    clf1 = GradientBoostingClassifier(n_estimators=200, min_samples_split=9)
    clf1.fit(train_feature[c_feature], target_list)
    preds1 = clf1.predict(test_feature)
    # right = 0
    for j in range(len(preds1)):
        size_result[test_id_list[j]] = preds1[j]
    return size_result
...
if __name__ == '__main__':
    ...
    clf = GradientBoostingClassifier(n_estimators=200, min_samples_split=17)
    clf.fit(trainFeature[cFeature], target)
    preds = clf.predict(testFeature)
    right = 0
    # for i in range(len(preds)):
    #     real = answer[testIdList[i]]['size']
